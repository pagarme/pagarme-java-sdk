
# Cursor Paging Response

Paggin response by foward cursor model (Pagging by foward cursor (https://docs.pagar.me/reference/pagina%C3%A7%C3%A3o-1#forward-cursor)

## Structure

`CursorPagingResponse`

## Fields

| Name | Type | Tags | Description | Getter | Setter |
|  --- | --- | --- | --- | --- | --- |
| `ForwardCursor` | `String` | Required | Pagging by foward cursor (https://docs.pagar.me/reference/pagina%C3%A7%C3%A3o-1#forward-cursor)<br><br>**Default**: `"null"` | String getForwardCursor() | setForwardCursor(String forwardCursor) |

## Example

```java
import me.pagar.api.models.CursorPagingResponse;

CursorPagingResponse cursorPagingResponse = new CursorPagingResponse.Builder(
    "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..5qM-BQbArZKXbfen5NnEXq6gbhyP-DrgsG1SMrpF4Y4"
)
.build();
```

